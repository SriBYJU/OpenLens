...
- Deployment source of truth: the latest successful `Verify and deploy OpenLens` workflow on `main`.
...
The reference twin and seven capability models execute locally. Manufacturer hardware records remain research-only; model execution is not a physical-device connection or hardware performance measurement. Simulation timing stays labeled. OCR is real local browser inference. Sign translation is deterministic curated lookup, not general machine translation.
