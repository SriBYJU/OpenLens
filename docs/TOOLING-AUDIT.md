...
Audit date: **2026-09-08**. Status distinguishes a downloaded source, a working local command, and a tool connected to the running Codex client. These are not interchangeable.
...
| Serena | [oraios/serena](https://github.com/oraios/serena) | No callable Serena MCP tool, `serena`, `uv`, or `uvx` found in the audit shell. Not installed or functionally tested here. Version is unverified; do not report one. Current official setup differs from old Git-based `uvx` recipes. |
| Atlas | [fkenmar/atlas](https://github.com/fkenmar/atlas) is the strongest match to the requested `atlas-map.md` workflow | No existing command found. Official release API has no stable release; latest published release is `v0.2.1-alpha`. No alpha binary installed because the directive asks for stable compatible tools. A hand-maintained map may satisfy navigation, but must not be represented as Atlas-generated. Exact identity from the prior Scholark session remains unconfirmed. |
...
The active callable-tool registry contained none of Serena, Atlas, Capsule or RTK. Local CLI/library invocation described above is explicit tool use outside that registry. Bounded searches in the available prior Scholark document folders did not establish the original Atlas/Capsule/Token Saviour identities. The linked projects match the directive's names and described behavior; this is an inference, not recovered history.
