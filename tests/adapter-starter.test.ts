...
describe('adapter starter generator',()=>{
 it('creates an implementation, contract test, runnable SDK example, verification record and guide',()=>{
  const files=buildAdapterStarter({deviceName:'Aurora One',deviceId:'aurora-one',capabilities:['camera','display']});
  expect(files.map(file=>file.path)).toEqual(['src/adapters/aurora-one.ts','src/adapters/aurora-one.test.ts','src/examples/openlens-sdk-example.ts','verification-record.json','README.md']);
  ...
  expect(files[2].content).toContain('return await openlens.device.execute');
  expect(JSON.parse(files[3].content)).toMatchObject({status:'unverified',adapterId:'aurora-one-adapter'});
 });
