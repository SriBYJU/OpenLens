...
export default function Lab(){
 ...
 const scenarioId=scenarios.find(s=>Object.entries(s.change).every(([key,value])=>work.config[key as keyof SimulationConfig]===value))?.id;
 ...
 return <Page className="instrument-page lab-page" index="01" eyebrow="LENS LAB / INTERACTIVE DIGITAL TWIN" title={<>The optical <em>workbench.</em></>} lead="Put an idea in the frame. Change the conditions. Inspect what happens." actions={<div className="instrument-status"><span className="live-dot"/> LOCAL SIMULATION<span>Seeded · repeatable · inspectable</span></div>}>
  ...
  <div className="workbench-grid">
   ...
   <div className="instrument-main"><section className="optical-console"><header><div><span className="live-dot"/>SIMULATION / {device.name.toUpperCase()}</div><button className="scene-run" disabled={device.kind==='research'} onClick={runScenario}>Run simulation ↗</button></header><div className={`optical-view ${work.run?.status??''}`}><FixtureScene fixture={work.config.fixture} environment={work.config.environment??defaultEnvironment}/><div className={`quality-gauge ${assessment.grade}`}><span>SYNTHETIC SIGNAL</span><strong>{assessment.score}/100</strong><i><b style={{width:`${assessment.score}%`}}/></i></div>{fov&&device.capabilities.display.physical==='present'?<div className="fov-frame"><i/><span>{fov.value}° {device.optics.fovAxis==='horizontal'?'H-FOV':device.optics.fovAxis==='diagonal'?'D-FOV':'FOV (axis unknown)'}</span><small>SCHEMATIC DISPLAY BOUNDARY</small></div>:<div className="no-display">NO DISPLAY PLANE<br/><small>This profile would need audio or companion output.</small></div>}<div className="lab-hud"><span>{work.experience.task.toUpperCase()} / {work.config.fixture.replace('-',' ').toUpperCase()}</span><strong aria-live="polite">{work.run?(work.run.output??(work.run.status==='failed'?'The run stopped.':'The plan is blocked.')):'Ready when you are.'}</strong><small>{work.run?`${work.run.status.toUpperCase()} · ${work.run.totalMs} MS · SEED ${work.run.seed}`:'RUN THE SCENARIO TO GENERATE OUTPUT'}</small></div><div className="view-meta"><span>{device.integrationStatus.toUpperCase()}</span><span>{work.experience.privacy.replace('-',' ').toUpperCase()}</span></div></div><div className="compile-strip">{plan.steps.map((step,index)=><div key={step.id} data-status={work.run?.trace.find(event=>event.stage===step.stage&&event.status!=='info')?.status??'pending'}><span>{index+1}. {step.stage}</span><strong>{step.route}</strong><small>{work.run?(work.run.trace.find(event=>event.stage===step.stage&&event.status!=='info')?.status??'not reached'):'planned'}</small></div>)}<b className={`compatibility ${plan.compatibility}`}>{plan.compatibility}</b></div></section>
