...
import {defaultSimulationConfig,experiencePresets,getDevice,type FailureMode,type SimulationConfig} from '../core';
...
export default function Lab(){
 ...
 const chooseScenario=(id:string)=>{const scenario=scenarios.find(item=>item.id===id)!;setScenarioId(id);work.setConfig({...work.config,battery:defaultSimulationConfig.battery,network:defaultSimulationConfig.network,permission:defaultSimulationConfig.permission,failureRate:0,...scenario.change})};
 ...
 return <Page index="01" eyebrow="LENS LAB / INTERACTIVE DIGITAL TWIN" title={<>Change a condition.<br/><em>See the system react.</em></>} lead="For product teams and developers testing a smart-glasses idea before hardware is available. Pick an experience and real-world condition, run it, then inspect exactly where it succeeded or failed." actions={<span className="truth-label simulated">RUNS IN THIS BROWSER</span>}>
  ...
  <p className="model-disclosure">{device.id.startsWith('model-')?device.summary:'The reference twin supports all modeled capabilities. Select a capability model to test missing sensors and output fallbacks.'}</p><SimulationControls/>
