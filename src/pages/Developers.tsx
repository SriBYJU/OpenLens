...
import AdapterConformanceLab from '../components/AdapterConformanceLab';
import FeatureGuide from '../components/FeatureGuide';
import Page from '../components/Page';
import SDKRuntimeLab from '../components/SDKRuntimeLab';
...
export default function Developers(){
 ...
 const guide=<FeatureGuide title="Connect hardware without overstating support" intro="OpenLens separates documented device features, executable adapter code, and physical verification. The generated bundle keeps each stage explicit." steps={[
   ...
   {number:'2',title:'Download five usable files',body:'Receive the adapter, contract test, runnable SDK example, verification record and exact integration steps.'},
 ...
 return <Page className="instrument-page developers-page" index="06" eyebrow="DEVELOPERS / SDK + ADAPTER WORKSHOP" title={<>Run the layer.<br/><em>Shape the adapter.</em></>} lead="Execute the source SDK against the reference twin, then describe a transport boundary and leave with code, tests, a working example, verification structure, and an implementation path." actions={<div className={`builder-state ${errors.length?'invalid':'ready'}`}><span>{errors.length?'NEEDS INPUT':'STARTER VALID'}</span><strong>{files.length}/5 files</strong></div>}>
  ...
  <div className="adapter-builder">
   ...
   <AdapterConformanceLab key={`${id}-${input.capabilities.join('-')}`} files={files} input={input}/>
