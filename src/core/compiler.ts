...
export function compileExperience(definition:ExperienceDefinition,device:DeviceProfile):CompiledPlan{
 ...
 const route=(capability:Capability,stage:'input'|'output',fallback=false):PlanStep=>{
  ...
  const reason=adapterCan?`${adapter!.name} exposes ${capability} in ${adapter!.mode} mode.`:`${device.name} is research-only in OpenLens; its manufacturer access claim does not create an implemented adapter.${chosen==='companion'?' A companion route is conceptually available, but OpenLens cannot execute this research profile.':''}`;
