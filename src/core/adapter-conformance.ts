...
const requiredPaths=(id:string)=>[`src/adapters/${id}.ts`,`src/adapters/${id}.test.ts`,'verification-record.json','README.md'];
...
export function evaluateAdapterBundle(files:AdapterStarterFile[],input:AdapterStarterInput,probe:ConformanceProbe='clean'):ConformanceReport{
 ...
 const checks:ConformanceCheck[]=[
  {id:'shape',label:'Bundle topology',detail:'Adapter, contract test, verification record, and guide are present at fixed paths.',passed:requiredPaths(input.deviceId).every(path=>bundle.some(file=>file.path===path))},
